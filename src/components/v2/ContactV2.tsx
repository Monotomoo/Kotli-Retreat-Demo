...
// V2 Contact — "Plan your journey", form v2.1.
// The form sits on a glass card; dropdowns replaced with selectable brass
// chips (every option visible, zero clicks wasted), labels warm to brass on
// focus. No backend exists yet (known issue) — submit composes a prefilled
// email in the visitor's mail client instead of posting to a dead endpoint.
const INK = '#0d120c';
const IVORY = '#f6f3ec';
const BRASS = '#b08d57';
...
const LOCAL_UI: Record<string, { caption: string; stepsLabel: string; steps: string[] }> = {
  de: {
...
export default function ContactV2() {
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const body = [
      `${t('form.firstName')}: ${String(data.get('firstName') ?? '—')}`,
      `${t('form.lastName')}: ${String(data.get('lastName') ?? '—')}`,
            ...
            >
              ...
              <form onSubmit={onSubmit} className="flex flex-col gap-7">
                ...
                <div>
                  <button
                    type="submit"
                    className="w-full py-4 text-[11px] font-semibold uppercase tracking-[2.5px] font-sans cursor-pointer border-none rounded-[3px] transition-colors duration-300"
